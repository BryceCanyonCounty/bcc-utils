...
function InstanceAPI.getInstancedCharacters(id)
    if not GameInstances[id] then
...
RPC:Register("CreateInstance", function(params, res, player)
    local id = InstanceAPI.create(params.id, player)
    return res(id)
end)

RPC:Register("LeaveInstance", function(params, res, player)
    InstanceAPI.leave(params.id, player)
    return res()
end)

RPC:Register("GetInstancedCharacters", function(params, res, player)
    local instances = InstanceAPI.getInstancedCharacters(player.id)
